...
public abstract class GpxJaxbContextHelper {
  ...
  /**
   * Parses the elemants of the given GPX extensions ({@link ExtensionsType#getAnies()}) with the given {@link
   * JAXBContext}:
   *
   * <p>If the {@link JAXBContext} can unmarshall the {@link Element} to a concrete object, a map
   * entry will be created with the class of the object as key and a list with all unmarshalled objects as value.
   *
   * <p>If the {@link JAXBContext} cannot unmarshall the {@link Element} to a concrete object, a
   * map entry will be created with the class of the element as key and a list with all elements as value.
   *
   * @param extensions the GPX extension
   * @param jaxbContext the {@link JAXBContext} to parse the elements
   * @return an unmodifiable map with the unmarshalled objects (key is the class of the objects, value is a list with
   *     all unmarshalled objects of this class)
   */
   ...
   *
   * @param cls the type
   * @param instancesOf if {@code true} instanceof will be used, otherwise {@link Class#equals(Object)} will be
   *     used
   ...
   *     JAXBContext)})
   * @param <T> the type
   * @return {@link Optional#empty()} if there is no such element, otherwise an optional with the parsed element
   */
